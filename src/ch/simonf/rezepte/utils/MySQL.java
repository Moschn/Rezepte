...
public class MySQL {
	...
	public boolean getUserDetails() throws JSONException
	{
		...
		else
		{
			...
			if(json.length() != 0)
			{
				...
				// only fetch ingredients if the user has any
	...
	{
		ArrayList<BasicNameValuePair> params = ingredient.toBasicNameValuePair();
		...
		{
			// created_at will be generated by the DB (default value current timestamp)
			if(params.get(i).getName() != "created_at")
			{
				namesList.add(params.get(i).getName());
				valuesList.add("'" + params.get(i).getValue() + "'");
			}
		}
		...
		if(!insert("ingredients", names, values))
	...
	private boolean insert(String table, String names, String values)
	{
		List<NameValuePair> params = new ArrayList<NameValuePair>();
		...
		Log.d("Insert " + table + ": ", json.toString());
	...
	private void simpleDialog(String message, String button1)
	{
	...
	/*public class TwoButtonDialog extends DialogFragment {
		private String message, button1, button2;
		
		private TwoButtonDialog(String message, String button1, String button2)
		{
